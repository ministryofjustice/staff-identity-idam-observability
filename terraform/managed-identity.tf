...
resource "azurerm_role_assignment" "assign_identity_dcr_T1_Perm_AccessPackages" {
  scope                = azurerm_monitor_data_collection_rule.data_collection_rule_T1_Perm_roles.id
  role_definition_name = "Monitoring Metrics Publisher"
  principal_id         = azurerm_user_assigned_identity.managed_identity.principal_id
...
resource "azurerm_role_assignment" "assign_identity_dcr_fido2_metrics_monitoring_publisher" {
  scope                = azurerm_monitor_data_collection_rule.data_collection_rule_fido2_metrics.id
  role_definition_name = "Monitoring Metrics Publisher"
  principal_id         = azurerm_user_assigned_identity.managed_identity.principal_id
